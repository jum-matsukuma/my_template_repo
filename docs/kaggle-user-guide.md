...
> **Note（実行手段の選択肢）**: 本ガイドの Colab 実行手順は「ブラウザでノートブックを開いて手動実行する」フローで書かれています。現在は公式 **Colab CLI**（ターミナルから GPU/TPU でヘッドレス実行）と **Colab MCP**（ブラウザノートブックのライブ操作）も利用可能で、Claude に「Colab で実行して」と頼むだけで CLI 経由の実行が使えます。詳細は [.claude/skills/colab/SKILL.md](../.claude/skills/colab/SKILL.md) を参照。以降のブラウザ手順はそのまま有効です。
...
詳細な手順は以下を参照：
- `.claude/skills/colab/SKILL.md` - Colab CLI / MCP による実行（GPU/TPU ヘッドレス実行・ノートブック操作）
- `.claude/skills/kaggle/colab-workflow.md` - Drive 構成・チェックポイント戦略（旧ブラウザ手動フロー含む）
