...
| `python` | `kaggle-template/`, `.claude/skills/kaggle/`, `.claude/commands/kaggle-update.md`（ML実験で Colab を使うなら `.claude/skills/colab/` は残す） |
...
- 削除したスキル/コマンドへの参照を除去（`.claude/skills/colab/` を削除した場合は `.claude/.mcp.json` の `colab` エントリも除去）
...
`.claude/skills/colab/` で Google 公式の 2 経路をサポート:

- **Colab CLI**（推奨・設定不要）— `uv tool install google-colab-cli` 後、`colab run` / `colab exec` で GPU/TPU ランタイム上のヘッドレス実行。リファレンスは `.claude/skills/colab/colab-operator.md`（公式 colab-operator スキルの vendored コピー）
- **Colab MCP**（opt-in）— ブラウザで開いた Colab ノートブックをライブ操作。`settings.local.json` の `enabledMcpjsonServers` に `"colab"` を追加して有効化。詳細は `.claude/skills/colab/colab-mcp.md`

Kaggle コンペでの使い分けは `.claude/skills/kaggle/SKILL.md` を参照。
